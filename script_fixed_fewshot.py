...
NL1 = "which rivers run through the state with the lowest elevation in the usa"
SQL1 = "SELECT river_name FROM river WHERE traverse IN ( SELECT state_name FROM highlow WHERE lowest_elevation  =  ( SELECT MIN ( lowest_elevation ) FROM highlow ) );"
NL2 = "What is the number of actors in the movie 'Saving Private Ryan' ?"
SQL2 = "SELECT COUNT ( DISTINCT t1.name  ) FROM CAST AS t2 JOIN actor AS t1 ON t2.aid  =  t1.aid JOIN movie AS t3 ON t3.mid  =  t2.msid WHERE t3.title  =  'Saving Private Ryan';"
...
def generate_query(state: SQLState):
    prompt = generate_query_system_prompt + "{}".format(state["init_query"])
    system_message = {
        "role": "system",
