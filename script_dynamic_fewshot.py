...
# filter the file to the database we are interested in.
...
exemplar_store = ExemplarStore(os.path.join(DATA_DIR, "train_spider.json"), num_examples=10)
...
def init_node(state: SQLState):
    nl = state["messages"][-1].content # should be the users query. 
    return {"init_query": nl, "test_result": "", "test_query": "", "final_query": "", "final_result": "", "rewrite": 0}
...
/* Examples  based on similar queries */

Answer the following question: {nl_query_1}
{sql_query_1}

Answer the following question: {nl_query_2}
{sql_query_2}

Answer the following question: {init_query}
"""

def generate_query(state: SQLState):

    # get dynamic examples
    #nl_queries = exemplar_store.get_similar_sql(state["init_query"], k=2)
    sql_queries = exemplar_store.get_similar(state["init_query"], k=2)
    #assert len(nl_queries) == 2, "did not find enough examples."

    
    #nl_query_1 = nl_queries[0].page_content
    #sql_query_1 = nl_queries[0].metadata.get('query')
    #nl_query_2 = nl_queries[1].page_content
    #sql_query_2 = nl_queries[1].metadata.get('query')

    sql_query_1 = sql_queries[0].page_content
    nl_query_1 = sql_queries[0].metadata.get('query')
    sql_query_2 = sql_queries[1].page_content
    nl_query_2 = sql_queries[1].metadata.get('query')

    system_message = {
        "role": "system",
        "content": generate_query_system_prompt.format(dialect=db.dialect, 
                                                       top_k=200,
                                                       nl_query_1=nl_query_1,
                                                       sql_query_1=sql_query_1,
                                                       nl_query_2=nl_query_2,
                                                       sql_query_2=sql_query_2,
                                                       init_query=state["init_query"]),
    }
